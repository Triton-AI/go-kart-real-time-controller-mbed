...
public:
  ...
  template <typename T> static std::shared_ptr<GkcPacket> CreatePacket() {
   ...
   */
  template <typename T>
  static GkcBuffer::iterator write_to_buffer(GkcBuffer::iterator where,
                                             const T &to_write) {
    const uint8_t *start =
   ...
   * @return GkcBuffer::const_iterator an iterator to the end of the read bytes
   * in the buffer
   */
  template <typename T>
  static GkcBuffer::const_iterator
  read_from_buffer(GkcBuffer::const_iterator where, T &to_read) {
